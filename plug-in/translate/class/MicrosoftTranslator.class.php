 ...
 * @param to $text
 */
public function translate($from, $to, $text, $format )
{
    $missing = 'missing';
    if(empty($to) || empty($text)) {
        $this->getErrorResponse($this->response, self::MISSING_ERROR, $missing);
        return;
    }
...
private function getSelectBox($response,$selectBox) {
    ...
    $select = "<select id ='".$selectBox['id']."' name='".$selectBox['name']. "'class='"."'>";
    $select.= $options;
    $select.= "</select>";
    return $select;
}
